...
import { getFletes, urlFor } from '../../lib/sanity'
  ...
  return (
    <section className="logistica section" id="logistica">
      <div className="container">
        ...
        <h2 className="section-title">Transportes <span>recomendados</span></h2>
        <p className="section-subtitle">Podés usar estas opciones o elegir el transporte que prefieras.
El envío se gestiona y abona directamente con el transporte, y si lo necesitás, te ayudamos a coordinarlo.</p>
          ...
          {fletes.map((flete) => (
            <div key={flete._id} className="logistica__card">
              <div className="logistica__img-wrap">
                {flete.logo ? (
                  <img
                    src={urlFor(flete.logo).width(300).height(150).fit('clip').url()}
                    alt={flete.nombre}
                    className="logistica__img"
                    loading="lazy"
                  />
                ) : (
                  <div className="logistica__img-placeholder">🚚</div>
                )}
              </div>
